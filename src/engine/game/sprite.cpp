...
namespace Blossom {
    ...
    void Sprite::drawNotAnimated(void) {
        Blossom::Vector4 rgbaColor = color;
    ...
    void Sprite::drawAnimated(void) {
        ...
        Blossom::Vector2 drawingBox = frame.boundingBox;
    ...
    void Sprite::setTextureFilter(Blossom::TextureFilter filter) {
        textureFilter = filter;

        if (isTextureLoaded()) {
            SetTextureFilter(texture, textureFilter == Blossom::TextureFilter::Linear ? TEXTURE_FILTER_BILINEAR : TEXTURE_FILTER_POINT);
        }
    }

    Blossom::TextureFilter Sprite::getTextureFilter(void) {
